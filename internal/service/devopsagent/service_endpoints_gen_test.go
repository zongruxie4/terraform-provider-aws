...
const (
	packageName = "devopsagent"
	awsEnvVar   = "AWS_ENDPOINT_URL_DEVOPS_AGENT"
	baseEnvVar  = "AWS_ENDPOINT_URL"
	configParam = "devops_agent"
)
...
func defaultEndpoint(ctx context.Context, region string) (url.URL, error) {
	r := devopsagent.NewDefaultEndpointResolverV2()

	ep, err := r.ResolveEndpoint(ctx, devopsagent.EndpointParameters{
...
func callService(ctx context.Context, t *testing.T, meta *conns.AWSClient) apiCallParams {
	...
	client := meta.DevOpsAgentClient(ctx)

	var result apiCallParams

	input := devopsagent.ListAgentSpacesInput{}
	_, err := client.ListAgentSpaces(ctx, &input,
		func(opts *devopsagent.Options) {
...
func expectDefaultEndpoint(ctx context.Context, t *testing.T, region string) caseExpectations {
	...
	if err != nil {
		t.Fatalf("resolving DevOps Agent default endpoint: %s", err)
	}
...
func expectDefaultFIPSEndpoint(ctx context.Context, t *testing.T, region string) caseExpectations {
	...
	if err != nil {
		t.Fatalf("resolving DevOps Agent FIPS endpoint: %s", err)
	}
	...
	if dnsErr, ok := errors.AsType[*net.DNSError](err); ok && (dnsErr.IsNotFound || dnsErr.IsTimeout) {
		...
		t.Fatalf("looking up DevOps Agent endpoint %q: %s", hostname, err)
	}
